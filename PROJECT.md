...
## Threat analysis (planned, app-side piece shipped 2026-09-10)
...
- **Status**: the app-side pieces above (ACL with kind/priority, auto-flagging, seed list, file-arrival watching + per-source trust, the opt-out) are shipped on `noxos-app` `main` and verified live on the local Cuttlefish v0 device (2026-09-10) — not just unit-tested. `AclRepository.nextAnalysisBatch()` (the priority-ordered, size-capped selection the future model would be fed from) exists and is tested but has no caller yet. The EC2 inference side (dataset, training, hosting, the actual model behind a flag) is **not started** — deliberately deferred to its own future session, scoped as real infra work (training pipeline + a live inference endpoint), not something to rush alongside the app-side wiring.
