...
## Threat analysis (app-side + dispatch loop shipped and verified end-to-end 2026-09-12; model/hosting still open)
...
- **User-facing opt-out**: a Settings toggle disables AI analysis entirely — when off, destinations never even get flagged as pending, since there'd be nothing to resolve the flag.
- **Status**: the app-side pieces (ACL with kind/priority, auto-flagging, seed list, file-arrival watching + per-source trust, the opt-out) are shipped on `noxos-app` `main` and verified live. **As of 2026-09-12, so is the dispatch loop**: `AnalysisDispatcher` (netmonitor module) polls `AclRepository.nextAnalysisBatch()` every 60s, POSTs each flagged entry to `<configured endpoint>/analyze/network`, and writes the verdict back via `AclRepository.allow()`/`.block()` — confirmed end-to-end on the v1 Cuttlefish device against a throwaway local responder standing in for a real model: flag → dispatch → real HTTP call → verdict → ACL write-back → the verdict actually enforcing on the next connection attempt. Settings UI exists for configuring the endpoint URL + API key. **What's still genuinely open**: there is no real model and no live EC2 endpoint — a separate repo, `noxos-inference` (`github.com/parrothacker1/noxos-inference`), exists with a README describing the intended `training/` + `service/` (FastAPI, XGBoost/LightGBM for network, ClamAV for files) + `infra/` (EC2 deploy, no IAM role needed) layout, but only `infra/`'s deploy scripts exist so far — no dataset work, no trained model, no deployed service. `/analyze/file` (the file-side endpoint) has no app-side caller yet either — only the network dispatch loop was built and wired.
