...
- **RESOLVED 2026-09-14: on-device vs. EC2 is not an either/or primary/fallback choice — it's a two-tier cascade.** The earlier framing ("which one is primary") was the wrong question; the real architecture is:
  ...
  **Status clarification, 2026-09-14 — "the on-device model is done" is true for one half of this, not the other.** Two genuinely separate things, easy to conflate:
  ...
  - **The model being an active feature on the phone: both real gaps now closed.** (1) `OnDeviceNetworkClassifier` was dead code until wired into `AnalysisDispatcher` as the real Tier 1 cascade stage (`e2b0581`, 2026-09-21) — an on-device `"allow"` now resolves the destination directly and EC2 is never contacted; anything else falls through to EC2 exactly as before. (2) Phase 1c (`noxos-os`, `MANAGE_VIRTUAL_MACHINE`) shipped and every VM-boot blocker underneath it is fixed (see the Session 18 update above) — the cheap filter that gates everything upstream of the classifier now genuinely works on real hardware, both for file-scan and network-sample-VM.
